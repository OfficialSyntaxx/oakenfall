/* What version this is, and what changed.
 *
 * GAME_VERSION is read by three things besides the game: the site build refuses
 * to publish if it disagrees with CHANGELOG.md's top entry, tools/
 * sync-native-version.mjs stamps it into the iOS and Android projects, and the
 * in-game diagnostics attach it to every bug report. Bump it here and nowhere
 * else.
 *
 * The list below is the player-facing changelog shown in the game. It is the
